...
def test_get_known_face_encodings_recalculates_when_cache_missing(mocker):
    mocker.patch("repository.repository._load_pickle", return_value=None)
    mocker.patch("repository.repository._calculate_know_face_encodings", return_value=(["e"], ["n"]))
...
def test_add_single_face_adds_and_persists_when_face_found(mocker, tmp_path, monkeypatch):
    monkeypatch.setattr(repository, "PICKLE_DIR", tmp_path)
